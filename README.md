...
- **取り込み（📥 取込）**: 外部 **DXF（R12）/ SVG** をパースして現在の図面に図形として追加（取り込み図形もサニタイズ適用）。SVGの曲線パス（C/S/Q/T/A）は線分近似で取り込み。transformは図形中心まわりのrotateのみ対応（中心以外を軸とするrotateや他のtransformは未対応）。stroke="none"は塗り色に矯正。DXFはエンティティ色（ACI/trueColor）とレイヤー（同名の層へ割当、無ければ作成）も取り込み。
...
node test/verify.mjs    # 機能テスト 221項目
