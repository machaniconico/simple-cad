...
const MIME = { '.html': 'text/html', '.js': 'text/javascript', '.svg': 'image/svg+xml', '.ico': 'image/x-icon', '.png': 'image/png', '.webmanifest': 'application/manifest+json', '.json': 'application/json' };
...
const manifestInstallMetadata = await page.evaluate(manifest => {
  const colorOk = value => typeof value === 'string' && /^#[0-9a-f]{6}$/i.test(value);
  ...
  return {
    ...
    themeMeta,
    themeColorOk: colorOk(manifest?.theme_color),
    backgroundColorOk: colorOk(manifest?.background_color),
  };
}, man);
check('manifestはインストール向け表示とテーマ情報を持つ',
  manifestInstallMetadata.display === 'standalone' &&
  manifestInstallMetadata.orientation === 'any' &&
  manifestInstallMetadata.shortName === 'SimpleCAD' &&
  manifestInstallMetadata.description.length >= 10 &&
  manifestInstallMetadata.themeColorOk === true &&
  manifestInstallMetadata.backgroundColorOk === true &&
  manifestInstallMetadata.themeMeta === manifestInstallMetadata.themeColor,
  JSON.stringify(manifestInstallMetadata));
...
const iconAssets = await page.evaluate(async () => {
  const icoEntrySizes = bytes => {
    const readU16 = offset => bytes[offset] | (bytes[offset + 1] << 8);
    const count = bytes.length >= 6 ? readU16(4) : 0;
    const sizes = [];
    const safeCount = Number.isFinite(count) ? Math.min(count, 64) : 0;
    for (let i = 0; i < safeCount; i++) {
      const base = 6 + i * 16;
      if (base + 16 > bytes.length) break;
      sizes.push(`${bytes[base] || 256}x${bytes[base + 1] || 256}`);
    }
    return sizes;
  };
  ...
  const readIco = async src => {
    const r = await fetch(src, { cache: 'no-store' });
    const blob = await r.blob();
    const bytes = new Uint8Array(await blob.arrayBuffer());
    const readU16 = offset => bytes.length >= offset + 2 ? bytes[offset] | (bytes[offset + 1] << 8) : -1;
    return {
      ok: r.ok,
      type: blob.type,
      size: blob.size,
...
check('manifestアイコンはany/maskable用途を明示する',
  manifestIconPurposes.length >= 3 && manifestIconPurposes.every(icon => icon.hasAny && icon.hasMaskable),
  JSON.stringify(manifestIconPurposes));
check('HTMLはICO fallback faviconを提供する',
  iconAssets.fallbackHref === 'simplecad.ico' &&
  iconAssets.fallbackType === 'image/x-icon' &&
  iconAssets.fallback.ok === true &&
  iconAssets.fallback.type === 'image/x-icon' &&
  iconAssets.fallback.reserved === 0 &&
  iconAssets.fallback.kind === 1 &&
  iconAssets.fallback.count >= 3 &&
  ['16x16', '32x32', '48x48'].every(size => iconAssets.fallback.sizes.includes(size)),
  JSON.stringify(iconAssets.fallback));
...
const offlineFallbackIcon = await page.evaluate(async () => {
  try {
    const r = await fetch('simplecad.ico');
    const blob = await r.blob();
    const bytes = new Uint8Array(await blob.arrayBuffer());
    const readU16 = offset => bytes.length >= offset + 2 ? bytes[offset] | (bytes[offset + 1] << 8) : -1;
    return {
...
check('オフラインでもICO fallback faviconをキャッシュから取得できる',
  offlineFallbackIcon.ok === true &&
  offlineFallbackIcon.type === 'image/x-icon' &&
  offlineFallbackIcon.size > 0 &&
  offlineFallbackIcon.reserved === 0 &&
  offlineFallbackIcon.kind === 1 &&
  offlineFallbackIcon.count >= 3,
  JSON.stringify(offlineFallbackIcon));
